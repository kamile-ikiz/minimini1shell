...
static void	init_expand_data(expand_data_t *data, char *arg, int dollar_pos)
{
...
static void	set_after_part(expand_data_t *data, char *arg, int var_end)
{
	data->after = ft_strdup(&arg[var_end]);
}

static void	create_result(expand_data_t *data, char *env_value)
{
	int	env_len;
...
static void	cleanup_expand_data(expand_data_t *data)
{
	if (data->before)
		free(data->before);
	if (data->after)
